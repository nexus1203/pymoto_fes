...
class FastEthServer:
    ...
    def __init__(self, ip="192.168.99.100"):
        """
        constructor method for FastEthServer class
        :param ip:  ip number of controller-server
        :return:    None
        """

        #create socket
        self.s = socket.socket(socket.AF_INET,          #Internet socket types
    ...
    def sendCmd2(self, reqSubHeader, reqData, procDiv=1, as_string = False):
        """
        Send Command (request packet) to Dx server ang get response (answer packet)
        :param reqSubHeader:    request sub header part of packet ( depend on each command )
        :param reqdata:         data part of the packet ( optional, depend of the command )
        ...
        # print(len(reqData))
        req_packet = UdpPacket_Req(reqSubHeader, reqData, procDiv)       #prepare packet
        req_packet.dataSize = [len(reqData), 0x00]
        req_packet.reqID=2
        # print(req_packet.dataSize)
        req_str = str(req_packet)                               #string representation of the packet

        ans_str = self.socketSndRcv(req_str)
        # print("reqData: ",req_str)
    ...
    def socketSndRcv(self, req_str):
        ...
        try:
            ...
            self.s.sendto( req_str.encode('latin-1'),                             #UDP packet
    ...
    def writeVar(self, types, index, value):
        ...
        reqSubHeader = { 'cmdNo': (commNo[types], 0x00),
                    'inst': [index, 0],
                    'attr': 1,
    ...
    def readVar(self, types, index):
        ...
        32bit integer Byte 0 Byte 1 Byte 2 Byte3 <Details>
        """
        reqSubHeader = { 'cmdNo': (commNo[types], 0x00), #cmd Nr
    ...
    def readVarCmd(self,cmd, types, index):
        ...
        32bit integer Byte 0 Byte 1 Byte 2 Byte3 <Details>
        """
        reqSubHeader = { 'cmdNo': (cmd, 0x00), #cmd Nr e.g. 0x77
                    'inst': [index, 0],       #index of var
                    'attr': 1,
                    'service':  0x0E,           #reading variable
                    'padding': (0, 0) }
    ...
    def FileList(self, filetype= "*.lst"):
        #Get File list
        ...
        ansPacket = self.sendCmd2(reqSubHeader, reqData, 2)
        # print(ansPacket)
    ...
    def FileDelete(self, filename):
        ...
        reqData = str(filename) # e.g. test.jbi
    ...
    #Save File from DX200 to PC
    def FileSave(self, filen, file_loc = ""):
        reqSubHeader = {    'cmdNo': (0x00, 0x00),
                            'inst': [0, 0],
                            'attr': 0,
                            'service':  0x16,
        ...
        ansPacket = self.sendCmd2(reqSubHeader, reqData, 2, as_string=True)
        # print(ansPacket.status)
        if ( ansPacket != None and ansPacket.status == 0):
            filedata = "".join(ansPacket.data)
            
            while True:
                ansPacket = self.sendCmd2(reqSubHeader, [], 2) # consecutive requests
                if ( ansPacket != None and ansPacket.status == 0):
                    filedata += "".join(ansPacket.data)
                else:
                    break
            with open(file_loc+filen, "w") as files:
                files.write(filedata)
            # print("File saved at : ", file_loc+filen)
            return True
        else:
            return False
    
    #Load File from PC to DX200
    def FileLoad(self, filen, file_loc = ""):
        reqSubHeader = {    'cmdNo': (0x00, 0x00),
                            'inst': [0, 0],
                            'attr': 0,
                            'service':  0x15,
        ...
        reqData = filen
        with open(file_loc+filen, "r") as files:
            txtfile = files.read()
        
        filedata = splitString(txtfile, chunksize=479)
        
        ansPacket = self.sendCmd(reqSubHeader, reqData, 2) # send code then start  writing
        # print(ansPacket.status)
        confirmation = 0
        if ( ansPacket != None and ansPacket.status == 8):
                        
            for data in filedata:
                ansPacket = self.sendCmd(reqSubHeader, data, 2) # consecutive writes
                confirmation+=1
                if ( ansPacket != None and ansPacket.status == 0):
                    a = 1
                else:
                    break
        print(len(filedata), confirmation)
        if confirmation== len(filedata):
            # print("File loaded at : ", filen)
            return True
        else:
            # print("Error occured while loading data to DX200")
            return False
